...
def resnet_v1(classes=2, depth=depth):
    ...
    x = Flatten()(x)
    y = Dense(classes, activation='softmax', kernel_initializer='he_normal', name='prediction')(x)
    model = Model(inputs=i, outputs=y, name='resnetv1')
    return model
...
def plot_loss(fit):
    plt.clf()
    plt_loss = plt
    plt_loss.plot(fit.history['loss'])
    plt_loss.plot(fit.history['val_loss'])
...
def plot_acc(fit):
    plt.clf()
    plt_acc = plt
    plt_acc.plot(fit.history['acc'])
    plt_acc.plot(fit.history['val_acc'])
